...
function BingoHeader({ label, isFirst, isLast }: BingoHeaderProps) {
  return (
    <div
...
function BingoCell({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
    >
      {value === 0 && isFree ? "FREE" : value}
    </div>
  );
}

function columnNumbers(start: number, end: number, count: number): number[] {
...
export default function BingoCard() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.localStorage) {
      ...
      setCardCount(stored ? Math.min(Number(stored), 2) : 1);
    }
  }, []);

  // Card keys for localStorage
  const getCardKey = (idx: number) => `bingoCardNumbers_${idx}`;
  const getSelectedKey = (idx: number) => `bingoCardSelected_${idx}`;

  // Generate card numbers
  const getInitialColumns = (idx: number) => {
    ...
    if (typeof window !== "undefined" && window.localStorage) {
      ...
      generated[2][2] = 0; // Use 0 as a placeholder for FREE
  ...
  const getInitialSelected = (idx: number) => {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && window.localStorage) {
      ...
      localStorage.setItem("bingoCardCount", cardCount.toString());
    }
  ...
  const checkWin = useCallback((sel: boolean[][], idx: number) => {
  ...
  const handleCellClick = useCallback(
    (rowIdx: number, colIdx: number, idx: number) => {
      if (colIdx === 2 && rowIdx === 2) return;
      setSelectedArr((prevArr) =>
        prevArr.map((sel, i) => {
          if (i !== idx) return sel;
          const newSelected = (sel ?? []).map((r: boolean[], rIdx: number) =>
            r.map((cell: boolean, cIdx: number) =>
